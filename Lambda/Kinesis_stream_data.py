...
def sendSMS(faceId, phone, otp):
    ...
    url = "http://smartdoorauthsystem.s3-website-us-east-1.amazonaws.com/OTPlogin.html?faceId="+faceId
...
def sendEmail(url,fileName):
    visitorUrl = "http://smartdoorauthsystem.s3-website-us-east-1.amazonaws.com/VisitorInfo.html?fileName="+fileName
        ...
        if face_search_response:
            ...
            for faces in face_search_response:
                if faces['MatchedFaces']:
                    ...
                    try :
                        item = vistiorsTable.get_item(Key={'FaceID': faceId})
    ...
    if success:
        if faceId is not None: 
            ...
            if not checkForDuplicates(passcodetable,faceId):
                ...
                vistiorsTable = connectToDB("visitors")
                print("Connected to visitors table")
